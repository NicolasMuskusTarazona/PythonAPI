...
def get_all_characters():
    return charactersEntity(conn.mydb.character.find())

# GET BY ID
@character.get('/characters/{id}', response_model=Character, tags=["characters"])
def get_id_characters(id: str):
    return characterEntity(conn.mydb.character.find_one({"_id": ObjectId(id)}))
...
def post_characters(character: Character):
    ...
    id = conn.mydb.character.insert_one(new_character).inserted_id
    character_created = conn.mydb.character.find_one({"_id": id})
...
def put_characters(id: str, characters: Character):
    put_character = conn.mydb.character.find_one_and_update(
...
def delete_characters(id: str, current_user: dict = Depends(get_current_admin)):
    deleted_character = conn.mydb.character.find_one_and_delete({"_id": ObjectId(id)})
